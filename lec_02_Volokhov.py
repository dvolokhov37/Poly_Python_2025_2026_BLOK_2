...
# Можно при разбиение добавить параметр axis для ориентации разбиения
...
# Поэтому используются векторизованных операций, которые можно распараллелить, т.к. они выполняются одновременно над каждым элементом массива
...
print(np.add(x, 1)) # соответсвует print(x + 1)
print(x + 1)
...
z = np.empty(len(x))
np.multiply(x, 10, out=z) # out - переменная, в которая поместим результат
print(z)
...
# Т.е. можно использовать как метод класса, так и функцию соответсвующего экземпляра класса
...
# которые позволяет уже [1, 2, 3] и [5, 5, 5] складывать между собой
# имеем [1 1 1      [0 1 2]  - здесь [0 1 2] растягивается вниз и будет [0 1 2
#        1 1 1   +                                                       0 1 2
#        1 1 1]                                                          0 1 2]
# также и со строками [1                              [1 1 1
#                      2   --> растягивается вправо    2 2 2
#                      3]                              3 3 3]
...
# то в массив с меньшей размерностью добавляется единичка с левой стороны (столька единичек, сколько нужно)
# т.е. [1, 3] -> [1, 3]; [1,] -> [1, 1]
...
# Итог: 2) для тех размеров, которые не равны, единицу превращаем в элемент соответсвующей размерности другого массива (расширение)
# 3) Если начинаем сравнивать зпачения по всем размерностям, и они не равны друг другу, и при этом ни один из них не равен 1,
